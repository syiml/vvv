...
public class AdminUserListHTML extends pageBean {
    ...
    String order;
    boolean desc;
    public AdminUserListHTML(int page,String search,String order,boolean desc){
        this.order=order;
        this.desc = desc;
    ...
    private String search(){
        ...
        if(desc) f.addForm(new hidden("desc","1"));
        f.setSubmitText("确定");
        return f.toHTML();
    }
...
//            }
    ...
    public String getLinkByPage(int page) {
        return "admin.jsp?page=UserAdmin&pa="+page+(search==null?"":"&search="+search)+(order==null?"":"&order="+order)+(desc ?"&desc=1":"");
    }
