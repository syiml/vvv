...
public class SomeOptRecordSQL {
     ...
     * @return 操作列表
     */
    public int getRecordNum(ESomeOptRecordType type,String user,Timestamp from,Timestamp to,Integer id){
        String unionString = "WHERE";
        List<Object> args = new ArrayList<>();
        StringBuilder sqlBuilder = new StringBuilder("SELECT count(*) FROM t_some_opt_record ");
        if(type != null){
            sqlBuilder.append(unionString).append(" `type`=? ");
            args.add(type.getValue());
            unionString = "AND";
        ...
        return new SQL(sqlBuilder.toString(),args).queryNum();
    }
